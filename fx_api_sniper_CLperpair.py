...
# - /health, /predict, /stats, /dashboard, /trade_event, /pnl_stats
#
# Run:
...
# If TradingView hint_side and ML side disagree, ML must be at least this confident
...
app = FastAPI(title="FX Sniper Per Pair", version="4.2-no-equity")
...
def stats():
    ...
    decision_counts = (
        ...
        if "decision" in df.columns else {}
    )

    pair_counts = (
        df["instrument"].value_counts(dropna=False).to_dict()
        if "instrument" in df.columns else {}
    )
...
def dashboard():
    ...
    if not trades_df.empty:
        pnl_cols = [c for c in ["ts", "instrument", "side", "units_signed", "status", "pnl", "order_id"] if c in trades_df.columns]
...
def trade_event(t: TradeEvent):
    ...
    return {"ok": True, "open_trades": current_open_trade_count()}
    ...
    try:
        ...
        out = make_out(
            ...
            why=(
                "OK"
                if would_order
                else f"Below sniper gate (conf>={conf_gate:.2f}, margin>={margin_gate:.2f})"
            ),
            ts=utc_ts(),
            pair=instrument,
            instrument=instrument,
